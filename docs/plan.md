...
- Khi vô hiệu hóa tài khoản:
    - có event realtime cho client.
    - Chat 1-1 không cho chat tiếp,
    - Group chat sẽ ẩn user bị vô hiệu hóa đó đi
    - Phần message thuộc về user đó trong chat 1-1 hoặc Group phải đổi thành 'Người dùng bị vô hiệu hoá'
    - phần list friends phải ẩn user đó đi, phần member có sự kiện realtime cho client.
    - Nếu đang online thì disconnect.
    - Các thông tin trên UI phải phản ánh đúng trạng thái này.

### 2. Hoàn thiện flow `cleanup-jobs`
